...
export class Plants {
  ...
  readonly filtered = computed(() => {
    ...
    return [...this.data.plants()].filter((plant) => !query || `${plant.code} ${plant.name} ${plant.location} ${plant.state} ${plant.contact}`.toLowerCase().includes(query))
      .sort((a, b) => {
  ...
  statusClass(status: string): string { return status === 'On target' ? 'success' : status === 'Behind' ? 'danger' : 'warning'; }
  ...
  save(): void {
    ...
    this.data.savePlant({ ...this.form.getRawValue(), id: this.editingId() ?? undefined }); this.view.set('list');
  }
  remove(plant: Plant): void { if (confirm(`Delete ${plant.name}? This cannot be undone.`)) this.data.deletePlant(plant.id); }
}
